...
const SKILL_MD = [
  ...
  "",
  "## Store secrets (global store, value via STDIN only)",
  "- `lockit set <slug> <KEY> [--schema <s>]` — store a secret field in the global store. The VALUE is read from STDIN only (never argv), so pipe it: `printf %s \"$VAL\" | lockit set stripe/prod STRIPE_KEY`. A slug can hold multiple fields.",
  "- `lockit set <slug> <KEY> --file` — store a FILE-type secret (a PEM/cert/JSON key file). See file-type fields below for how these inject.",
  "",
  "## Per-project keys",
  ...
  "- `lockit admit <NAME...> | <slug#field> [--as <NAME>] [--force]` admits one or more stored keys — whole secrets by name, or a single `slug#field`. It requires ONE human presence confirmation — a macOS Touch ID / account-password dialog (a terminal prompt where that is unavailable). An agent cannot satisfy it, so only a human approves. You may REQUEST it — explain clearly which keys and why. `--as` binds a key under a different env-var name; `--force` overwrites an existing `.env` entry.",
  ...
  "",
  "## Use the secrets",
  "- `lockit run -- <cmd>` — inside a project, run a command with the project's admitted keys injected in memory and masked in output; you never see them. Works in both modes.",
  "- `lockit run <slug> -- <cmd>` — outside a project, run a command with one global secret's fields injected. (Inside a project this global form is refused; use `run -- <cmd>`.)",
  "",
  "## File-type fields (`--file`)",
  "- A file-type field is NOT injected as a value. On `lockit run`, lockit materializes it to a fresh temporary file at mode 0600 and sets its env var to that file's PATH; the file is shredded when the command exits.",
  "- So pass the env var (a path) to the program that needs the file — e.g. `lockit run certs/prod -- myserver --tls-cert \"$TLS_CERT\"`. NEVER `cat`, read, or print that file: its contents are the secret. The path itself is safe to see.",
  ...
  "- `lockit import [<path>] [--as <slug>]` — import a `.env` file into the encrypted store (default `./.env`). Does not modify the source file.",
  "- `lockit export [--out <file>]` — write a VALUE-FREE reference file (`.env.ref` with `KEY=@ref` lines) that is safe to commit to git so a teammate's own keys fill it. Never contains values.",
  "- `lockit pull <VAR...> | <bundle#VAR> | --all <bundle> [--out <file>] [--force]` and `lockit resolve [<ref-file>] [--out <file>] [--force]` WRITE REAL PLAINTEXT secret values to disk and require a human local-auth gate. AVOID both for agent work — prefer admit + `run`.",
  "",
  ...
  "- `lockit pull` and `lockit resolve` write plaintext to disk (human-gated); prefer admit + `run`.",
  ...
  "- `lockit lock` clears the unlock session now, so the next store command re-prompts for Touch ID.",
