...
describe("kv set (e2e, real binary)", () => {
  ...
  it("defaults the schema to the slug's first segment", async () => {
    await withSandbox(async (home) => {
      const set = await runKv(home, ["set", "supabase/acme", "DB_URL"], {
        passphrase: PW,
        stdin: "postgres://x",
      });
      expect(set.code).toBe(0);
      const ls = await runKv(home, ["ls"], { passphrase: PW });
  ...
  it("--schema overrides the default schema", async () => {
    await withSandbox(async (home) => {
      const set = await runKv(home, ["set", "supabase/acme", "DB_URL", "--schema", "postgres"], {
        passphrase: PW,
        stdin: "v",
      });
      expect(set.code).toBe(0);
  ...
  it("--schema with an empty value is rejected (exit 1, usage on stderr)", async () => {
    await withSandbox(async (home) => {
      const set = await runKv(home, ["set", "openai/dev", "K", "--schema", ""], {
        passphrase: PW,
        stdin: "v",
      });
  ...
  it("--file marks the field as file type (shown via the confirmation line)", async () => {
    await withSandbox(async (home) => {
      const set = await runKv(home, ["set", "openai/dev", "TOKEN", "--file"], {
        passphrase: PW,
        stdin: "filevalue",
      });
      expect(set.code).toBe(0);
  ...
  it("stores multiple fields on one secret (multi-field)", async () => {
    await withSandbox(async (home) => {
      const a = await runKv(home, ["set", "openai/dev", "ALPHA"], { passphrase: PW, stdin: "a" });
      const b = await runKv(home, ["set", "openai/dev", "BRAVO"], { passphrase: PW, stdin: "b" });
  ...
  it("missing KV_PASSPHRASE is a clear error with exit 1", async () => {
    await withSandbox(async (home) => {
      // Empty passphrase exercises the same 'not set' guard, deterministically,
      // independent of whatever the host env may carry.
      const set = await runKv(home, ["set", "openai/dev", "K"], {
        stdin: "v",
        env: { KV_PASSPHRASE: "" },
      });
      expect(set.code).toBe(1);
      expect(set.stderr).toContain("KV_PASSPHRASE is not set");
      expect(set.stdout).toBe("");
    });
  });
  ...
  it("usage error when slug and key are missing (exit 1)", async () => {
    await withSandbox(async (home) => {
      const set = await runKv(home, ["set"], { passphrase: PW, stdin: "v" });
      expect(set.code).toBe(1);
      expect(set.stderr).toContain("usage: kv set <slug> <KEY>");
    });
  });

  it("usage error when only the slug is given (exit 1)", async () => {
    await withSandbox(async (home) => {
      const set = await runKv(home, ["set", "openai/dev"], { passphrase: PW, stdin: "v" });
      expect(set.code).toBe(1);
      expect(set.stderr).toContain("usage: kv set <slug> <KEY>");
    });
  });
  ...
  it("rejects an invalid field key (leading digit) with exit 1", async () => {
    await withSandbox(async (home) => {
      const set = await runKv(home, ["set", "openai/dev", "1BADKEY"], {
        passphrase: PW,
        stdin: "v",
      });
      expect(set.code).toBe(1);
  ...
  it("writes no plaintext to disk and creates the store at 0600 under $KV_HOME", async () => {
    await withSandbox(async (home) => {
      const secret = "sk-on-disk-PLAINTEXT-CHECK";
      const set = await runKv(home, ["set", "openai/dev", "OPENAI_API_KEY"], {
        passphrase: PW,
        stdin: secret,
      });
      expect(set.code).toBe(0);
  ...
  it("a wrong passphrase refuses to decrypt the store later", async () => {
    await withSandbox(async (home) => {
      const set = await runKv(home, ["set", "openai/dev", "K"], { passphrase: PW, stdin: "v" });
      expect(set.code).toBe(0);

      const ls = await runKv(home, ["ls"], { passphrase: "the-WRONG-passphrase" });
  ...
  it("upsert replaces a field value without duplicating it", async () => {
    await withSandbox(async (home) => {
      await runKv(home, ["set", "openai/dev", "K"], { passphrase: PW, stdin: "first" });
      await runKv(home, ["set", "openai/dev", "K"], { passphrase: PW, stdin: "second" });
      const ls = await runKv(home, ["ls"], { passphrase: PW });
