...
describe("kv ls (e2e, real binary)", () => {
  ...
  it("lists a single-field secret value-free as '<slug>  [<schema>]  <KEY>'", async () => {
    await withSandbox(async (home) => {
      ...
      await runKv(home, ["set", "openai/dev", "OPENAI_API_KEY"], {
        passphrase: PW,
        stdin: secret,
      });
      const ls = await runKv(home, ["ls"], { passphrase: PW });
  ...
  it("shows multiple field keys comma-separated in insertion order, never a value", async () => {
    await withSandbox(async (home) => {
      await runKv(home, ["set", "openai/dev", "ALPHA"], { passphrase: PW, stdin: "v-alpha" });
      await runKv(home, ["set", "openai/dev", "BRAVO"], { passphrase: PW, stdin: "v-bravo" });
      await runKv(home, ["set", "openai/dev", "CHARLIE"], { passphrase: PW, stdin: "v-charlie" });
      const ls = await runKv(home, ["ls"], { passphrase: PW });
      expect(ls.code).toBe(0);
  ...
  it("uses exactly two spaces around the [schema] token", async () => {
    await withSandbox(async (home) => {
      await runKv(home, ["set", "openai/dev", "K", "--schema", "custom"], {
        passphrase: PW,
        stdin: "v",
      });
      const ls = await runKv(home, ["ls"], { passphrase: PW });
  ...
  it("lists multiple secrets one per line, each terminated by a newline", async () => {
    await withSandbox(async (home) => {
      await runKv(home, ["set", "openai/dev", "K"], { passphrase: PW, stdin: "a" });
      await runKv(home, ["set", "stripe/live", "K"], { passphrase: PW, stdin: "b" });
      const ls = await runKv(home, ["ls"], { passphrase: PW });
      expect(ls.code).toBe(0);
  ...
  it("never reveals a value even for a file-type field", async () => {
    await withSandbox(async (home) => {
      ...
      await runKv(home, ["set", "openai/dev", "TOKEN", "--file"], {
        passphrase: PW,
        stdin: secret,
      });
      const ls = await runKv(home, ["ls"], { passphrase: PW });
  ...
  it("missing KV_PASSPHRASE is a clear error with exit 1", async () => {
    await withSandbox(async (home) => {
      const ls = await runKv(home, ["ls"], { env: { KV_PASSPHRASE: "" } });
      expect(ls.code).toBe(1);
      expect(ls.stderr).toContain("KV_PASSPHRASE is not set");
      expect(ls.stdout).toBe("");
    });
  });
  ...
  it("a wrong passphrase refuses to decrypt (exit 1, value-free error)", async () => {
    await withSandbox(async (home) => {
      await runKv(home, ["set", "openai/dev", "K"], { passphrase: PW, stdin: "v" });
      const ls = await runKv(home, ["ls"], { passphrase: "WRONG-passphrase" });
      expect(ls.code).toBe(1);
