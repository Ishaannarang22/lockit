...
**Goal:** Ship the `plugin/` Claude Code plugin — a manifest, an agent-safe-usage skill, and a `PreToolUse` egress-guardrail hook — as a thin, unprivileged layer over the `kv` CLI that teaches agents to *use* secrets via `kv run` without ever seeing values, and warns/blocks before a raw secret is written into a file or command.

**Depends on:** Plan #5 (the `kv` CLI: `run`, `status`, `list`, `link`, the admission flow, the chooser, `--dry-run`). The plugin is sugar over that binary and adds no privileged path. No MCP — see [ADR 0005](../../adr/0005-drop-mcp-cli-and-plugin.md).

**Packages touched:** `plugin/` (new top-level directory at the edge of the dependency graph; depends only on the `kv` CLI invoked as a subprocess). No changes to `crypto`, `core`, or `server`. The hook is implemented as a small TypeScript module so it can be unit-tested with vitest under the existing workspace toolchain; whether it ships as its own `@kv/plugin-hooks` workspace package or as plain scripts under `plugin/` is an expansion-time decision (see Open questions).
...
- **The agent-safe-usage skill**: a `SKILL.md` (plus frontmatter) that teaches an agent the correct mental model and workflow — `kv` lets you *use* secrets without *seeing* them. It instructs the agent to: discover requirements with `kv status` / `kv list` / `kv run --dry-run` (value-free), **request** admission (never assume it), understand that admission needs a human + local auth the agent cannot satisfy, and **run programs under `kv run -- <cmd>`** rather than reading any value or copying it into a file, `.env`, or shell command.
...
- **Content lints** that keep the skill honest and aligned with the canonical invariants (no instruction to ever read a value; always prefers `kv run`; references the human-gated admission gate).
...
- `plugin/skills/agent-safe-kv/SKILL.md` — the skill content: agent-safe `kv` mental model, the request-admission flow, and the "use `kv run`, never read the value" rule.
...
- `plugin/skills/agent-safe-kv/skill-content.test.ts` — content lints asserting the skill says the right things and never the wrong ones.
...
The reason string names only the finding *label* and a remediation hint ("looks like a raw secret is about to be written to `config.ts`; use `kv run -- …` so the value is injected at runtime instead") — it never echoes the matched secret. The executable wrapper handles stdin/stdout/exit-code plumbing and stays a thin shell so the decision logic is unit-testable in isolation.

**Skill (`SKILL.md`).** Prose + frontmatter teaching the agent: the agent never sees values; discover requirements value-free; request admission and wait for the human to confirm with local auth; run everything under `kv run -- <cmd>`; never paste a value into a file/`.env`/command; treat an `AMBIGUOUS` chooser as a question for the human, not something to guess. It must point at the CLI as the only interface and explicitly note the plugin adds no privileged access.
...
- **File-write egress is caught:** a `PreToolUse` event for a Write/Edit whose content contains a raw secret yields a `block` (or `warn`) decision, with a value-free reason that recommends `kv run` — proving a raw secret on its way into a file is intercepted.
...
- **Clean input passes:** a `PreToolUse` event whose content is `kv run -- npm start` or otherwise contains no secret yields `allow` with no false positive — proving the guardrail does not block the *correct* agent-safe workflow.
- **`kv run` invocations are never flagged:** a command that uses `kv run -- …` (the sanctioned path, where the value is injected at runtime and never present in the command text) is allowed — proving the hook rewards the safe pattern rather than punishing it.
- **Manifest validity:** `plugin.json` parses as JSON, has the required plugin fields, registers the `agent-safe-kv` skill and the `PreToolUse` hook, points the hook at an existing entrypoint, and **declares no MCP server** — proving ADR 0005 is honored at the manifest level.
- **Skill content lint — uses `kv run`, never reads values:** the skill text instructs the agent to run programs via `kv run -- <cmd>` and contains no instruction to read, print, copy, cat, or echo a secret value; an assertion fails if any "read the value" pattern appears — proving the skill enforces the agent-never-sees-a-value invariant.
...
- **Unprivileged path:** a check that the skill and README describe the plugin as flowing entirely through the `kv` binary with no privileged access — proving the plugin adds guardrails, not a bypass.
...
- **Auto-remediation** (rewriting a flagged Write into a `kv run` invocation). The hook warns/blocks and advises; it does not silently edit the agent's action.

## Open questions

- **Workspace-package vs scripts:** should the hook ship as a `@kv/plugin-hooks` workspace package (clean build/test boundary) or as plain TS/JS under `plugin/` wired into the root vitest `include`? Decide at expansion time based on how Claude Code resolves a hook executable path.
