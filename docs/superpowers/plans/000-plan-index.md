...
The packages form an inward dependency graph: `@lockit/crypto` is the pure, I/O-free trust root;
`@lockit/core` builds the data + sandbox layer on top of it; `@lockit/cli` wraps `core`; the Claude
...
- **Scope:** Stand up the pnpm/TypeScript monorepo (vitest, eslint, CI) and build the `@lockit/crypto` at-rest foundation: Argon2id key derivation, XChaCha20-Poly1305 AEAD, the versioned sealed-blob format, and passphrase seal/open.
...
- **Status:** INTENDED.
- **Scope:** Build the asymmetric sharing trust root in `@lockit/crypto`: X25519 keypair generation, the HKDF-SHA256 seed-triple expansion, HPKE Auth-mode wrap/unwrap of a per-item DEK, Ed25519 sign/verify, and the age-style multi-recipient envelope.
...
- **Status:** INTENDED.
- **Scope:** Build `@lockit/core`'s data layer: the encrypted slug-keyed global store of Secrets, the value-free project vault of Slots, the built-in schema registry, the strict 0/1/N resolver, the per-environment axis, and the gitignored local resolution cache.
- **Dependencies:** Plan #1 (`@lockit/crypto` at-rest seal/open). Asymmetric crypto is NOT required.
...
- **Status:** INTENDED.
- **Scope:** Build the project-world sandbox (the admitted set), the human-gated admission flow with a pluggable `AuthProvider`, the `lockit run` in-memory injection engine (env + file secrets, output masking, dry-run preview), the audit log, and the agent-safe listing surface.
...
- **Status:** INTENDED.
- **Scope:** Build the `lockit` binary — the verb-first command surface over `@lockit/core` for secret/slot/link/status/run commands — where values enter only via prompt or stdin (never argv), every agent-facing output is value-free, and `share` is a stable deferred stub.
- **Dependencies:** Plan #3 (`@lockit/core` store/vault) and Plan #4 (`@lockit/core` sandbox + admission). Transitively Plan #1.
- **Proving tests:** Integration tests that build and spawn the real compiled `lockit` binary in a temp HOME/project — values from argv are refused, stdin path leaks nothing, `ls`/`status`/`--dry-run` are value-free, ambiguous resolution is a structured chooser, injection at `run` masks child output and keeps values out of the parent process and `lockit`'s own output, plus a pinned exit-code contract.

### 6. Claude Plugin
- **Filename:** `2026-06-17-p6-claude-plugin.md`
- **Status:** INTENDED.
- **Scope:** Ship the `plugin/` Claude Code plugin — a manifest, an agent-safe-usage skill, and a `PreToolUse` egress-guardrail hook (provider-pattern + high-entropy detector) — as a thin, unprivileged layer over the `lockit` CLI that teaches agents to use secrets via `lockit run` without seeing values.
- **Dependencies:** Plan #5 (the `lockit` CLI it wraps as a subprocess).
- **Proving tests:** Provider-pattern and high-entropy detection without false positives or value leakage, file-write and command egress caught while clean input and `lockit run` invocations pass, manifest validity, and skill content lints (uses `lockit run` / never reads values, human-gated admission, honest containment limit).
...
- **Dependencies:** Plan #2 (asymmetric envelope) and Plan #3 (`@lockit/core` store + reference resolution).
...
- **The CLI gets integration tests that spawn the binary.** Plan #5 builds and spawns the real compiled `lockit` binary (temp HOME, temp project), asserting output masking at `run` and that every agent-facing output — listings, status, dry-run, the ambiguous chooser, and `lockit`'s own non-child output — never contains a secret value.
