...
| Feature                     | Behavior to test                               | Input / command                                                                                                                 | Expected output                                                | Exit code | Test layer  |
| --------------------------- | ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------- | --------- | ----------- |
| Create vault on first write | Creates .lockit/vault.json when none exists        | `vault.addSlot({ schema: "openai", bind: "pinned", to: "openai/main", inject: { "API_KEY": "OPENAI_API_KEY" } }); vault.save()` | .lockit/vault.json created with valid JSON; file contains one slot | 0         | integration |
| Read existing vault         | Loads existing .lockit/vault.json                  | Existing .lockit/vault.json with 2 slots                                                                                            | `vault.listSlots()` returns array with 2 slots                 | 0         | integration |
...
| Remove slot from vault      | Deletes slot by index or identity              | `vault.removeSlot(0); vault.save()`                                                                                             | Slot gone from `listSlots()`; .lockit/vault.json updated           | 0         | integration |
...
| Reject malformed JSON       | Errors when .lockit/vault.json is invalid JSON     | .lockit/vault.json contains `{ "slots": [unclosed`                                                                                  | Throws parse error or validation error                         | 1         | integration |
| Empty vault on missing file | Returns empty vault when .lockit/vault.json absent | No .lockit/vault.json file exists                                                                                                   | `vault.listSlots()` returns []                                 | 0         | integration |
...
| Feature                            | Behavior to test                                                     | Input / command                                                                                                  | Expected output                                                                                            | Exit code | Test layer  |
| ---------------------------------- | -------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------- | --------- | ----------- |
| Create cache on first write        | Creates .lockit/local.json when none exists                              | `cache.recordFill({ slot, schema: "openai", slug: "openai/main", env: "dev", resolvedAt: "..." }); cache.save()` | .lockit/local.json created with valid JSON                                                                     | 0         | integration |
| Read existing cache                | Loads existing .lockit/local.json                                        | .lockit/local.json exists with 1 cache entry                                                                         | `cache.listEntries()` returns array with 1 entry                                                           | 0         | integration |
...
| Cache records slugs only           | No field values stored in .lockit/local.json                             | Record fill with secret containing field values                                                                  | Cache contains slug only; no field values on disk                                                          | 0         | integration |
...
| Reject malformed cache JSON        | Errors when .lockit/local.json is invalid JSON                           | .lockit/local.json contains `{ "entries": [unclosed`                                                                 | Throws parse error                                                                                         | 1         | integration |
...
| Lazy stale cleanup                | Stale entry not deleted from disk immediately                                 | Cache entry for deleted slug remains in .lockit/local.json after load                                   | Entry stays until next explicit cleanup or expiry                 | 0         | integration |
...
| Value change picked up next run        | Rotating a secret means all slots get the new value on next injection       | Secret rotated; `lockit run` re-executes                                                | Injected env vars reflect new rotated value                   | 0         | integration |
