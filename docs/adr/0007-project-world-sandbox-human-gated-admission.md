...
and casual channels. A core goal of **lockit** is that **AI agents can use secrets
...
batch. There is **no re-auth** on later `lockit run` (re-auth-per-use is an optional
...
triggers **lazily** at `lockit run` / `lockit status` — never on `git clone`. There is
**no daemon and no filesystem watcher** (an optional opt-in direnv-style `cd`
hook may come later).

**Injection (`lockit run`).** `lockit run` decrypts the needed secrets **in memory
...
and shred it. `lockit run --dry-run` prints the env-var **names** that will be set
...
- Values never enter model context or the transcript; agents work with slugs and
  booleans.
- `lockit run --dry-run` lets an agent verify what will happen without seeing any
  value.
...
- **A daemon or filesystem watcher for eager resolution** — convenient, but it
  ...
  Rejected for v1; lazy resolution at `lockit run`/`lockit status` only, with an
