...
something that releases a decryption key. Read literally, every fresh `kv`
...
- **Passphrase is the root.** `AK = Argon2id(passphrase)`. The store payload is
  ...
  (`wrapKey`/`unwrapKey` in `@kv/crypto`). The DEK indirection lets the unlock
...
- **The cache is an encrypted, Touch-ID-gated keychain item.** After one
  ...
  policy. Auto-lock (sleep / idle timeout / `kv lock`) evicts it.
...
- **Off-device falls back to the passphrase.** SSH types it; CI supplies it as a
  ...
  against.
- **The crypto root is a dedicated kv passphrase**, never the OS account
...
- **The cache is only as strong as the Secure Enclave**, and exists only on
  ...
  explicitly weaker.
- **Layering.** `@kv/crypto` ships `wrapKey`/`unwrapKey` now; the DEK-wrapped
  store lands in P3; the keychain cache, auto-lock, LocalAuthentication provider,
  and per-use vs per-session policy land in P4; passphrase/`KV_PASSPHRASE`
